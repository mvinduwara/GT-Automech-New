...
import { Category, UnitOfMeasure } from '@/types/types';
...
export default function Create() {
    const { categories, unitOfMeasures, success, error } =
        usePage<CreateStockPageProps>().props;
    ...
    useEffect(() => {
        if (success) {
            ...
            setMainProductSearchQuery('');
            setAltProductSearchQuery('');
        }
        if (error) {
            toast.error(error);
        }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('dashboard.stock.store'));
    };
