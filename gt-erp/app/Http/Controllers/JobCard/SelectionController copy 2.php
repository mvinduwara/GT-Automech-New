    ...
    public function oilBrands(): JsonResponse
    {
        ...
        try {
            ...
            $brands = Brand::whereHas('products.stocks')
                ->whereHas('products.category', fn($q) => $q->where('id', $categoryId))
                ->select('id', 'name')
                ->orderBy('name')
                ->get();
    ...
    public function oils(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $oils = Stock::with(['product' => fn($q) => $q->with(['brand', 'category'])])
                ->whereHas('product.category', fn($q) => $q->where('id', $categoryId))
                ->whereHas('product.brand', fn($q) => $q->where('id', $request->brand_id))
                // ->where('status', 'active')
                ->get()
                ->map(function ($stock) {
                    // Here we can map the result to a new collection with the required attributes.
                    // We can access the selling_price from the stock record itself.
                    return (object) [
                        'id' => $stock->product->id,
